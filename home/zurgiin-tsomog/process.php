...
include('config.php');
	...
	if($action == 'zuragadd'){
		if($id == ''){
			$qry = "
				insert into zurag(body,photo)
				values ('{$_POST['body']}','{$file_name}')
				";
		}else{
			$qry = "
				update zurag set
		...
		if ($conn->query($qry) === true) {
            echo "<script type='text/javascript'>window.top.location='/home/zurgiin-tsomog/update-zurag.php';</script>"; exit;
		}else{
			echo $conn->error;
        }
    }
    if($action == "zuragdel"){
        ...
        $qry = "delete from zurag where id = '{$id}'";
        if ($conn->query($qry) === true) {
            echo "<script type='text/javascript'>window.top.location='/home/zurgiin-tsomog/update-zurag.php';</script>"; exit;
        }else{
            echo $conn->error;
