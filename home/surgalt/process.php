...
include('config.php');
	...
	if($action == 'surgaltadd'){
		if($id == ''){
			$qry = "
				insert into surgalt(title, body,photo,type)
				values ('{$_POST['title']}','{$_POST['body']}','{$file_name}','{$_POST['type']}')
				";
		}else{
			$qry = "
				update surgalt set
    ...
    if($action == "surgaltdel"){
        $id = isset($_GET['id']) ? $_GET['id'] : '';
        $qry = "delete from surgalt where id = '{$id}'";
        if ($conn->query($qry) === true) {
            echo "<script type='text/javascript'>window.top.location='/home/welcome.php';</script>"; exit;
        }else{
            echo $conn->error;
