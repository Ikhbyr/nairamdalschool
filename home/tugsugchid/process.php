...
include('config.php');
	...
	if($action == 'tugsugchadd'){
		if($id == ''){
			$qry = "
				insert into tugsugch(ner, ovog, amjilt, body, photo ,year)
				values ('{$_POST['ner']}','{$_POST['ovog']}','{$_POST['amjilt']}','{$_POST['body']}','{$file_name}',{$_POST['year']})
				";
		}else{
			$qry = "
				update tugsugch set
		...
		if ($conn->query($qry) === true) {
            echo "<script type='text/javascript'>window.top.location='/home/tugsugchid/update-tugsugch.php';</script>"; exit;
		}else{
			echo $conn->error;
        }
    }
    if($action == "tugsugchdel"){
        $id = isset($_GET['id']) ? $_GET['id'] : '';
        $qry = "delete from tugsugch where id = '{$id}'";
        if ($conn->query($qry) === true) {
            echo "<script type='text/javascript'>window.top.location='/home/tugsugchid/update-tugsugch.php';</script>"; exit;
        }else{
            echo $conn->error;
