...
&& $imageFileType != "gif" && $imageFileType != "jpg") {
    echo "Уучлаарай, Таны оруулсан зураг JPG, JPEG, PNG & GIF зөвхөн эдгээр өргөтгөлтэй байх ёстой.";
    $uploadOk = 0;
}
...
include('config.php');
	...
	if($action == 'medeeadd'){
		if($id == ''){
			$qry = "
				insert into tuuh(title, body, photo,year)
				values ('{$_POST['title']}','{$_POST['body']}','{$file_name}','{$_POST['year']}')
				";
		}else{
			$qry = "
				update tuuh set
    ...
    if($action == "medeedel"){
        ...
        $qry = "delete from medee where id = '{$id}'";
        if ($conn->query($qry) === true) {
            echo "<script type='text/javascript'>window.top.location='/home/mendchilgee/update-medee.php';</script>"; exit;
        }else{
            echo $conn->error;
        }
    } 
